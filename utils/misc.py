...
def random_sample_points_pytorch(points, sample_nums, with_idx=False):
    """pytorch版的下采样，将对点云进行随机下采样

    Args:
        points (torch.Tensor): 需要被下采样的点云
        sample_nums (int): 下采样点数

    Returns:
        torch.Tensor: 经过下采样的点
    """
    num_points = points.shape[0]
    if num_points == sample_nums:
        return points, torch.arange(num_points) if with_idx else points
    elif num_points < sample_nums:
        choice = torch.arange(num_points)
        extra_num = sample_nums - num_points
        extra_choice = torch.randint(0, num_points+1, (extra_num, 1)).squeeze(1) - 1
        choice = torch.cat((choice, extra_choice))
        return points[choice], choice if with_idx else points[choice]
    elif num_points > sample_nums:
        extra_num = sample_nums
        choice = torch.randint(0, num_points+1, (extra_num, 1)).squeeze(1) - 1
        return points[choice], choice if with_idx else points[choice]
