...
from utils.timm_compat import DropPath, trunc_normal_
...
class PCTransformer(nn.Module):
    def __init__(self, config):
        ...
        self.class_num = config.get("class_num", None)
        self.config = config
        ...
        # Memory settings
        self.memory_size = config.get("memory_size", None)
        if self.memory_size is None:
            self.memory_size = config.class_num
        class_num = config.get("class_num", None)
        self.memory_per_class_num = (
            self.memory_size // class_num if class_num is not None else None
        )
        ...
        # Keep the original BN behavior for memory_size > 1. With memory_size=1,
        # memory vectors produce [1, C, 1], so BatchNorm1d cannot train.
        memory_mlp_bn = self.memory_size > 1
        self.memory_query_mlp = MLP_CONV(in_channel=config.encoder_config.embed_dim, layer_dims=[384, config.encoder_config.embed_dim], bn=memory_mlp_bn)
        self.memory_key_mlp = MLP_CONV(in_channel=config.encoder_config.embed_dim, layer_dims=[384, config.encoder_config.embed_dim], bn=memory_mlp_bn)
        self.memory_value_mlp = MLP_CONV(in_channel=config.decoder_config.embed_dim, layer_dims=[384, config.decoder_config.embed_dim], bn=memory_mlp_bn)
        self.key_similar_size = config.get("key_similar_size", 4)
    ...
    def forward(self, input_dict):
        ...
        input_dict["num_query"] = torch.tensor(self.num_query).cuda()
        ...
        if self.training:
            ...
            input_dict["denoise_length"] = torch.tensor(denoise_length).cuda()
            ...
            input_dict["q"] = q[:, 1:]
            input_dict["coarse_points"] = coarse
            input_dict["denoise_length"] = torch.tensor(0).cuda()
