...
from utils.timm_compat import DropPath
...
def knn_point(nsample, xyz, new_xyz, transposed=False):
    """
    Args:
        nsample (int): number of nearest neighbors.
        xyz (torch.Tensor): (B, N, 3) if transposed == False, else
            (B, 3, N). xyz coordinates of the features.
        new_xyz (torch.Tensor, optional): (B, npoint, 3) if transposed
    ...
    Returns:
        torch.Tensor: (B, npoint, k) tensor with the indices of the
        features that form k-nearest neighbours.
    """
    group_idx = knn(nsample, xyz.contiguous(), new_xyz.contiguous(), transposed).transpose(1, 2)
...
class DynamicGraphAttention(nn.Module):
    ...
    def forward(self, q, q_pos, v=None, v_pos=None, idx=None, denoise_length=None):
        ...
        if denoise_length is None:
            ...
            # first query a neighborhood for one query token
            if idx is None:
                idx = knn(self.k, v_pos, q_pos).transpose(1, 2) # B N k 
            assert idx.size(-1) == self.k
            # gather the neighbor point feat
