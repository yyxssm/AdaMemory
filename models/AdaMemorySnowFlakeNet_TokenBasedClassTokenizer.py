...
class AdaMemorySnowFlakeNet_TokenBasedClassTokenizer(nn.Module):
    ...
    def forward(self, input_dict, return_P0=False):
        ...
        if self.training:
            ...
            gt_cls_token_norm = gt_cls_token.norm(
                float('inf'), dim=[1], keepdim=True
            ).clamp_min(1.0e-8)
            gt_cls_token = gt_cls_token * (
                1 / torch.sqrt(torch.tensor(512)) / gt_cls_token_norm
            )
        ...
        cls_token_norm = cls_token.norm(
            float('inf'), dim=[1], keepdim=True
        ).clamp_min(1.0e-8)
        cls_token = cls_token * (
            1 / torch.sqrt(torch.tensor(512)) / cls_token_norm
        )
