...
sys.path.append(os.path.join(os.path.dirname("__file__"), '..', '..'))
from models.FSCSVD_utils import *
# from metrics.CD.chamfer3D.dist_chamfer_3D import chamfer_3DDist
from extensions.chamfer_dist import ChamferFunctionWithIdxNoGrad as chamfer_3DDist
...
class SDG(nn.Module):
    def __init__(self, channel=128,ratio=1,hidden_dim = 512,dataset='ShapeNet'):
        ...
        self.conv_11 = nn.Conv1d(1024, 256, kernel_size=1)
    ...
    def forward(self, local_feat, coarse,f_g,partial):
        ...
        f_g_repeat = f_g.repeat(1, 1, F.shape[-1])
        F = torch.cat([F.contiguous(), f_g_repeat.contiguous()], dim=1)
        ...
        F_Q = self.sa1(F, embd)
        F_Q_ = self.decoder1(F_Q).contiguous()

        # Similarity Alignment
        local_feat = self.mlpp(local_feat)
        F_H = self.cross1(F_Q.contiguous(), local_feat)
        F_H_ = self.decoder2(F_H).contiguous()
...
class SVFNet(nn.Module):
    ...
    def get_img_layers(backbone, feat_size):
        ...
        from models.FSCSVD_resnet import _resnet, BasicBlock
...
class local_encoder(nn.Module):
    ...
    # def __init__(self):
        ...
        self.local_number = cfg.local_points
        # self.local_number = 64
