...
class AdaMemoryPCN(nn.Module):
    def __init__(self, config):
        ...
        self.feature_align = MLP_CONV(in_channel=256 + 256, layer_dims=[512, 256])
        self.classifier = MLP_CONV(in_channel=config.decoder_config.embed_dim, layer_dims=[128, config.class_num])
        self.feature_align2 = MLP_CONV(in_channel=config.decoder_config.embed_dim, layer_dims=[512, 1024])
    ...
    def get_compactness_loss(self, input_dict):
        ...
        gt_points_token = input_dict["gt_points_token"].squeeze(2)
    ...
    def forward(self, input_dict):
        ...
        feature_global = torch.cat((feature_global.squeeze(2), gnn_feature), dim=1).unsqueeze(2)
        feature_global = self.feature_align(feature_global)
        ...
        if self.training:
            ...
            input_dict["gt_points_token"] = gt_feature_global
        ...
        semantic_aware_feat = self.feature_align2(semantic_aware_feat.unsqueeze(2)).squeeze(2)
