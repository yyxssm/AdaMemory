...
from .SnowFlakeNet_utils import PointNet_SA_Module_KNN, MLP_Res, MLP_CONV, fps_subsample, grouping_operation, query_knn
...
class Decoder(nn.Module):
    ...
    def forward(self, feat, partial, return_P0=False, semantic_aware_feat=None):
        ...
        for upper in self.uppers:
            ...
            K_prev = K_prev + semantic_aware_feat
...
class AdaMemorySnowFlakeNet_EncoderBasedClassTokenizer(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.fc2 = nn.Linear(256, config.encoder_config.embed_dim)
        self.bn_fc1 = nn.BatchNorm1d(256)
        self.bn_fc2 = nn.BatchNorm1d(config.encoder_config.embed_dim)
        self.feature_align = MLP_CONV(in_channel=1024, layer_dims=[384, 512])
        self.classifier = MLP_CONV(in_channel=config.encoder_config.embed_dim, layer_dims=[128, config.class_num])
    ...
    def get_class_constraint(self, input_dict):
        """
        Classification constraint loss.
        Args:
            input_dict: Dictionary containing input data.
        Returns:
            ce_loss: Cross entropy loss for classification.
        """
    ...
    def forward(self, input_dict, return_P0=False):
        ...
        gnn_feature = self.gnn1_embed(point_cloud, adj, None)
        gnn_feature = torch.max(gnn_feature, 1)[0]
        gnn_feature = self.bn_fc2(self.fc2(F.relu(self.bn_fc1(self.fc1(gnn_feature))))).unsqueeze(2)
        pred_class_label = self.classifier(gnn_feature).squeeze(2)
        input_dict["pred_class_label"] = pred_class_label
        
        if self.training:
            f = self.feat_extractor(input_dict["gt_points"].transpose(2, 1).contiguous())  # B 512 n
            f = f * (1 / torch.sqrt(torch.tensor(512)) / f.norm(float('inf'), dim=[1], keepdim=True))
            input_dict["gt_points_token"] = f
        ...
        feat = self.feat_extractor(point_cloud)  # b c num_points
        feat = feat * (1 / torch.sqrt(torch.tensor(512)) / feat.norm(float('inf'), dim=[1], keepdim=True))
        input_dict["encoder_cls_token"] = feat
        ...
        feat = torch.cat((feat, gnn_feature), dim=1)
