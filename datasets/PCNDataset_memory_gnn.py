...
from PCN_synset_dict import config
...
class PCNMemoryGNN(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return int(config["remap"][sample['taxonomy_id']]), sample['model_id'], data['partial'], data['gt'], adj

    def __len__(self):
        return len(self.file_list)

@DATASETS.register_module()
class PCNv2MemoryGNN(data.Dataset):
    # def __init__(self, data_root, subset, class_choice = None):
    def __init__(self, config):
        self.partial_points_path = config.PARTIAL_POINTS_PATH
        self.complete_points_path = config.COMPLETE_POINTS_PATH
