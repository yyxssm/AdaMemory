...
#     gt_label = [item[2][2].clone().detach() for item in batch]
...
#     gt_label = pad_sequence(gt_label, batch_first=True)

#     return taxonomy_ids, model_ids, (partial_data, gt_data, gt_label)
...
def collate_fn(batch):
    ...
    # # 使用 pad_sequence 对 gt_data 进行填充
    gt_data = pad_sequence(gt_data, batch_first=True)
    # gt_label = pad_sequence(gt_label, batch_first=True)

    return taxonomy_ids, model_ids, gt_data


# V2X代码，仅用于重建的函数
def collate_fn_v2x_rebuild(batch):
    taxonomy_ids = [item[0] for item in batch]
    model_ids = [item[1] for item in batch]
    partial_data = [item[2][0].clone().detach() for item in batch]
    gt_data = [item[2][1].clone().detach() for item in batch]
    ...
    return taxonomy_ids, model_ids, (partial_data, gt_data)



def collate_fn_no_seg(batch):
    taxonomy_ids = [item[0] for item in batch]
    model_ids = [item[1] for item in batch]
    partial_data = [item[2][0].clone().detach() for item in batch]
    gt_data = [item[2][1].clone().detach() for item in batch]
